param(
    [ValidateSet('all','product','product-core','product-add','product-price','product-list','account', 'cart')]
    [string]$Scope = 'all'
)
...
$filter = switch ($Scope) {
    ...
    'cart' { 'FullyQualifiedName~HomeControllerCartTests' }
    default   { $null }
}

# Run tests and produce TRX
$dotnetArgs = @('test','1298_DuyHung.csproj','--logger','trx;LogFileName=testResults.trx','--results-directory',$resultsDir,'--verbosity','minimal','--no-build')
if ($filter) {
...
$descriptions = @{
    ...
    # HomeControllerCartTests
    'AddToCart_WithNewProduct_ReturnsRedirectToIndex' = @{ Case = 19; Input = "ProductId=1, Quantity=2"; Output = "Redirect to Index"; };
    'RemoveFromCart_WhenItemExists_ReturnsSuccessJson' = @{ Case = 20; Input = "ProductId=1 (exists)"; Output = "JSON success=true"; };
    'GetCartCount_ReturnsTotalQuantityOfItems'        = @{ Case = 21; Input = "Cart[Item1(qty:3) + Item2(qty:2)]"; Output = "JSON count=5"; };
}
